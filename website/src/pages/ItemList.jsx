  ...
  return (
    <div className="item-list-page animate-fade-in section container">
      <div className="text-center" style={{ marginBottom: '3rem' }}>
        <h1 className="section-title">Your Inquiry List</h1>
        <p className="section-subtitle">Review your selected items before proceeding to contact us.</p>
      </div>

      {inquiryItems.length === 0 ? (
        <div className="empty-state text-center" style={{ padding: '4rem', background: '#fffdfa', borderRadius: '20px', border: '1px solid #E5DED0' }}>
          <h3 style={{ fontFamily: 'var(--font-serif)', marginBottom: '1rem', color: 'var(--color-primary)' }}>Your list is currently empty</h3>
          <p style={{ opacity: 0.8, marginBottom: '2rem' }}>Browse our collections to add items to your wholesale inquiry.</p>
          <Link to="/" className="btn">Return to Collections</Link>
        </div>
      ) : (
        <div className="inquiry-content split-layout">
          ...
          <div className="inquiry-summary" style={{
            ...
            <div style={{ paddingBottom: '1.5rem', borderBottom: '1px solid #E5DED0' }}>
              <h3 style={{ color: 'var(--color-primary)', marginBottom: '0.5rem', fontSize: '1.5rem', fontFamily: 'var(--font-serif)' }}>Ready to proceed?</h3>
              <p style={{ color: 'var(--color-text)', opacity: 0.8 }}>Total items requested: <strong style={{ color: 'var(--color-primary)' }}>{getTotalItems()}</strong></p>
            </div>
      ...
      {isModalOpen && createPortal(
        <div className="modal-overlay" style={{
          ...
          <div className="modal-content" style={{
            ...
            <h2 style={{ fontFamily: 'var(--font-serif)', color: 'var(--color-primary)', marginBottom: '1.5rem', fontSize: '2rem' }}>Inquiry Form</h2>
            ...
            <form onSubmit={async (e) => {
              ...
              {showSuccess ? (
                <div style={{ textAlign: 'center', padding: '3rem 1rem' }}>
                  ...
                  <p style={{ opacity: 0.8, marginBottom: '2rem', fontSize: '1.1rem' }}>We'll be in touch soon.</p>
                  <button className="btn btn-full" onClick={() => { setShowSuccess(false); setIsModalOpen(false); }} type="button">Close</button>
                </div>
              ) : (
                <>
                  <div style={{ marginBottom: '2rem', padding: '1.5rem', background: 'var(--color-primary)', color: 'var(--color-bg)', borderRadius: '12px' }}>
                    <h4 style={{ marginBottom: '1rem', borderBottom: '1px solid rgba(255,255,255,0.2)', paddingBottom: '0.5rem', color: 'var(--color-bg)' }}>Requested Items</h4>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginBottom: '2rem' }}>
                    <input type="text" name="name" placeholder="Name" required style={{ padding: '1rem', borderRadius: '8px', border: '1px solid #E5DED0', width: '100%', fontFamily: 'inherit' }} />
                    <input type="email" name="email" placeholder="Email" required style={{ padding: '1rem', borderRadius: '8px', border: '1px solid #E5DED0', width: '100%', fontFamily: 'inherit' }} />
                    <input type="tel" name="mobile" placeholder="Phone Number" style={{ padding: '1rem', borderRadius: '8px', border: '1px solid #E5DED0', width: '100%', fontFamily: 'inherit' }} />
                    <textarea name="message" placeholder="Message / Additional Instructions" required style={{ padding: '1rem', borderRadius: '8px', border: '1px solid #E5DED0', width: '100%', minHeight: '120px', fontFamily: 'inherit', resize: 'vertical' }}></textarea>
                  </div>
